...
#Creating empty sets to contain pairs of UMI and true strand position per line for each strand type
pos_strand_umi_true_position_set = set()
neg_strand_umi_true_position_set = set()

#Initializing variables to start iterating and counting over chromosomes
chromosome_label = "1"
    ...
    with open(args.file,"r") as input_sam:
        ...
        for line in input_sam:
            ...
            #Alignment line logic
                ...
                #If the current line's chromosome does not match the currently iterated chromosome label, empty the UMI sets for each strand, print current chromosome label & count,
                #then set chromosome label to current line's value and resent count to 0.
                if chromosome_label != split_line[2]:
                    
                    pos_strand_umi_true_position_set = set()
                    neg_strand_umi_true_position_set = set()
        
                    print(f"Count of reads per chromosome {chromosome_label}: {count_of_reads_per_chromosome}")
...
print(f"Number of header lines = {header_linecount}")
print(f"Number of unique reads in output file = {unique_read_count}")
